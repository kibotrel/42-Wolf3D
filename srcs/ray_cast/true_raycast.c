...
/*   Updated: 2019/05/07 16:45:38 by reda-con         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_ray_cast	check_coll(t_ray_cast all, int **map)
{
	...
	{
		all = check_neg(all);
		if (hit_x == 0 && map[(int)all.coll_x.y / SQUARE_SIZE][(int)all.coll_x.x / SQUARE_SIZE] != 0)
			hit_x = 1;
		else if (hit_x == 0)
		{
		...
		if (hit_y == 0 && map[(int)all.coll_y.y / SQUARE_SIZE][(int)all.coll_y.x / SQUARE_SIZE] != 0)
		{
			hit_y = 1;
		}
		else if (hit_y == 0)
...
void				true_raycast(int **map, SDL_Renderer *ren)
{
	...
	while (++x < WIN_WIDTH)
	{
		...
		all.dist_col = (sqrt(pow(all.coll_x.x - play.play_coor.x, 2) + pow(all.coll_x.y - play.play_coor.y, 2)) > sqrt(pow(all.coll_y.x - play.play_coor.x, 2) + pow(all.coll_y.y - play.play_coor.y, 2)))\
			? sqrt(pow(all.coll_y.x - play.play_coor.x, 2) + pow(all.coll_y.y - play.play_coor.y, 2)) : sqrt(pow(all.coll_x.x - play.play_coor.x, 2) + pow(all.coll_x.y - play.play_coor.y, 2));
