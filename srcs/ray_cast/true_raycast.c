...
/*   Updated: 2019/05/07 13:52:42 by reda-con         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		true_raycast(int **map, SDL_Renderer *ren)
{
	...
	while (++x < WIN_WIDTH)
	{
		...
		all.dist_col = (sqrt(pow(all.coll_x.x - play.play_coor.x, 2)\
					+ pow(all.coll_x.y - play.play_coor.y, 2))\
				> sqrt(pow(all.coll_y.x - play.play_coor.x, 2)\
					+ pow(all.coll_y.y - play.play_coor.y, 2)))\
					   ? sqrt(pow(all.coll_y.x - play.play_coor.x, 2)\
							   + pow(all.coll_y.y - play.play_coor.y, 2))\
					   : sqrt(pow(all.coll_x.x - play.play_coor.x, 2)\
							   + pow(all.coll_x.y - play.play_coor.y, 2));
